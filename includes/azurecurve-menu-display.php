...
if ( ! function_exists( 'azrcrv_display_azurecurve_menu' ) ) {
	function azrcrv_display_azurecurve_menu() {
		...
		echo '<div id="azrcrv-general" class="wrap">';
			...
			$tab_active_label = esc_html__( 'Active Plugins', 'azrcrv-spg' );
			$active_plugins   = '';
		...
		foreach ( $plugin_array as $plugin_name => $plugin_details ) {
				...
				<table class="form-table azrcrv-settings">
					...
					<tr>
						...
						<td scope="row" colspan=2>
							...
							. '</p>
							<p>' .
							sprintf( esc_html__( 'The %s plugins active on your site are:', 'azrcrv-spg' ), '<strong>azurecurve | Development</strong>' )
							. '</p>
			...
			$tab_other_label = esc_html__( 'Other Available Plugins', 'azrcrv-spg' );
			$other_plugins   = '';
		...
		if ( $countofplugins == 0 ) {
			$other_plugins .= sprintf( esc_html__( 'Congratulations! You\'re using all of the %s plugins.', 'azrcrv-spg' ), 'azurecurve | Development' );
		}
				...
				<table class="form-table azrcrv-settings">
					...
					<tr>
						...
						<td scope="row" colspan=2>
							...
							<p>' .
							sprintf( esc_html__( 'The other plugins available from %s are:', 'azrcrv-spg' ), '<strong>azurecurve | Development</strong>' )
							. '</p>
